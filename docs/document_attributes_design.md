...
文集中的文档在加载入数据库时，需要指定一个类目，以便 agent 在读取文档列表时可以按照特定类目分类读取。

文档的类目采用树型结构，并用扁平化的“路径”的方式来进行表示，类似文件系统的结构。例如："法律法规/采购管理"，"行业规章/综合管理/投资管理" 等。
...
| 前序版本 | replaces    | str(100)  |    是     | 前序版本的标题，必须一致               |
| 发布机构 | pub_path    | str(100)  |    否     | 文档发布机构路径，详见组织机构相关说明 |
| 上级文档 | localizes   | str(100)  |    是     | 上级文档的标题，必须一致               |
| 原文作者 | authors     | str(100)  |    是     | 多名作者用逗号分隔                     |
| 文档类目 | category    | str(500)  |    是     | (仅insert时) 文档类目路径              |
| 类目ID   | category_id | uuid-v7   |    否     | (仅update时) 文档类目ID                |
...
- **特别注意**: `update` 标注中要修改一个文档的类目时，必须使用该类目的ID，而非名称或路径，类目ID可以使用命令行工具 `hurag categories` 查看。
...
`corpus.json` 中标注的文档类目路径如果在数据库中不存在，则在加载该文档时会设置为未分类（无类目）。

## 加载

使用 CLI 命令 `hurag load` 加载文集，完成新文档入库、修改文档元数据、删除库内文档等操作。加载过程中自动完成向量化文档的分割、内嵌和多模态文档（包括附件）的内容提取。
...
*虽然加载文集时可进行自动分割，但更推荐在加载前使用 `corpus split` CLI 命令预先完成分割，尤其是存在需要手动分割的文档时，以便分割后可以人为进行一次检查，确保分割无误。*
